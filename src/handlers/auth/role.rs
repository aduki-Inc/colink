...
pub async fn create_role(
	...
	let claims = match ext.get::<Claims>() {
		Some(claims) => claims,
		None => { return HttpResponse::BadRequest().json(json!({ "success": false, "message": "Authorization failure!"})) }
	};
	...
	let role = match role_data.validate() {
		Ok(role) => role,
		Err(err) => { return HttpResponse::ExpectationFailed().json(json!({"success": false, "message": err.to_string()})); }
	};

	// Check if the Role already exists
	if role_exists(&role.author, &role.section, &mut conn) {
		return HttpResponse::Conflict().json(json!({ "success": false, "message": "Same role already exists!"}));
	}
	...
	match role_created(&new_role, &mut conn) {
		Ok(role) => {
			...
			let new_log = new_section_log(
				user.id,
				role.section,
				ActionType::Create, format!("{} created a new role", &user.username)
			).await;

			// Spawn an independent task(Record log)
			spawn(async move { create_log(&new_log, &mut conn).await });
			
			HttpResponse::Ok().json(json!({ "success": true, "role": role, "message": format!("Role - ({}) - was created", &role.name)})) },
		Err(Error::DatabaseError(DatabaseErrorKind::ForeignKeyViolation, _)) => {
			HttpResponse::NotFound().json(json!({ "success": false, "message": "Section or User does not exists"}))
		},
		Err(_) => {
			// Handle the database error and return an error response
			HttpResponse::InternalServerError().json(json!({"success": false,"message": "Failed to create the role: Internal Error Occurred!"}))
		}
	}
}
...
pub async fn update_privileges(
    ...
    let authorized = match check_authority(&user.id, &role.section, &role.base, &mut conn).await {
	    Ok(authorized) => authorized,
	    Err(_) => { return HttpResponse::InternalServerError().json(json!({"success": false,"message": "Internal server error has occurred!"})); }
    };
    ...
    let updated_role = match privileges_updated(&role, &mut conn) {
	    ...
	    Err(Error::NotFound) => { return HttpResponse::NotFound().json(json!({"success": false,"message": "No such role was  found!"})); },
	    Err(_) => { return HttpResponse::InternalServerError().json(json!({"success": false,"message": "Internal server error has occurred!"})); }
    };
