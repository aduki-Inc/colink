...
use crate::db::schema::{roles, orgs as org_model, approvals, sections};
use crate::models::{
  orgs::{Organization, InsertableOrganization}, 
...
pub fn org_exists(unique_name: &str, org_name: &str, conn: &mut PgConnection) -> Result<bool, Error> {
  match orgs.filter(short_name.eq(unique_name).and(name.eq(org_name))).get_result::<Organization>(conn) {
    Ok(_) => Ok(true),
